...
def preprocess_all_new(input_file, output_folder, window_size=20):
    ...
    text_str_all = re.sub(r'[^a-zA-Z ]', '', text_str_all.lower())
    words_all = text_str_all.split()
    
    text_str_narr = content['text_narr']
    text_str_narr = re.sub(r'[^a-zA-Z ]', '', text_str_narr.lower())
    words_narr = text_str_narr.split()
    
    text_str_qa = content['text_qa']
    text_str_qa = re.sub(r'[^a-zA-Z ]', '', text_str_qa.lower())
    words_qa = text_str_qa.split()
...
def process_scores_file(seperate_file, processed_folder, political_bigrams, allwords1):
    """处理单个文件并计算scores"""
    scores = {}
    try:
        # 第一步：生成中间处理文件
        ...
        # 计算无条件分数
        deceppos = len([word for word in words if word in allwords1['positive']])
        decepneg = len([word for word in words if word in allwords1['negative']])
        deceppos_narr = len([word for word in words_narr if word in allwords1['positive']])
        decepneg_narr = len([word for word in words_narr if word in allwords1['negative']])
        deceppos_qa = len([word for word in words_qa if word in allwords1['positive']])
        decepneg_qa = len([word for word in words_qa if word in allwords1['negative']])
        
        # 初始化scores
        scores[title] = {
            'Deceptive': deceppos-decepneg,
            'Deceptive_narr': deceppos_narr-decepneg_narr,
            'Deceptive_qa': deceppos_qa-decepneg_qa,
            'CSR_1':0,
            'CSR_2':0,
            'CSR_3':0,
            'CSR_1_narr':0,
            ...
            'CSR_Deceptive_2': 0,
            'CSR_Deceptive_3': 0,
            'CSR_Deceptive_1_narr': 0,
            'CSR_Deceptive_2_narr': 0,
            'CSR_Deceptive_3_narr': 0,
            'CSR_Deceptive_1_qa': 0,
            'CSR_Deceptive_2_qa': 0,
            'CSR_Deceptive_3_qa': 0,
        ...
        for window in windows1:
            # Find middle ngram and check whether a "political" unigram
            middle_bigram = window[10] if len(window) > 10 else None
            if middle_bigram and middle_bigram in political_bigrams:
                tfidf1 = political_bigrams[middle_bigram]['tfidf']
                
                # Create word list for easy and quick access
                window_words1 = set([y for x in window for y in x.split()])
                
                # If yes, check whether positive or negative sentiment
                conditional_deceppos = len([word for word in window_words1 
                                         if word in allwords1['positive']])
                conditional_decepneg = len([word for word in window_words1 
                                         if word in allwords1['negative']])
                
                # Weigh by tfidf
                conditional_deceppos = conditional_deceppos * tfidf1
                conditional_decepneg = conditional_decepneg * tfidf1
                
                # Collect results
                scores[title]['CSR_1'] += tfidf1
                scores[title]['CSR_Deceptive_1'] += (conditional_deceppos - conditional_decepneg)
        ...
        for window in windows2:
            ...
            if middle_bigram and middle_bigram in political_bigrams:
                ...
                if middle_bigram1 not in political_bigrams and middle_bigram2 not in political_bigrams:
                    tfidf2 = political_bigrams[middle_bigram]['tfidf']
                    
                    # Create word list for easy and quick access
                    window_words2 = set([y for x in window for y in x.split()])
                    
                    # If yes, check whether positive or negative sentiment
                    conditional_deceppos = len([word for word in window_words2 
                                             if word in allwords1['positive']])
                    conditional_decepneg = len([word for word in window_words2 
                                             if word in allwords1['negative']])
                    
                    # Weigh by tfidf
                    conditional_deceppos = conditional_deceppos * tfidf2
                    conditional_decepneg = conditional_decepneg * tfidf2
                    
                    # Collect results
                    scores[title]['CSR_2'] += tfidf2
                    scores[title]['CSR_Deceptive_2'] += (conditional_deceppos - conditional_decepneg)

        # 处理trigram_windows
        for window in windows3:
            # Find middle ngram and check whether a "political" trigram
            middle_bigram = window[10] if len(window) > 10 else None
            if middle_bigram and middle_bigram in political_bigrams:
                ...
                if middle_bigram3 not in political_bigrams and middle_bigram4 not in political_bigrams:
                    tfidf3 = political_bigrams[middle_bigram]['tfidf']
                    
                    # Create word list for easy and quick access
                    window_words3 = set([y for x in window for y in x.split()])
                    
                    # If yes, check whether positive or negative sentiment
                    conditional_deceppos = len([word for word in window_words3 
                                             if word in allwords1['positive']])
                    conditional_decepneg = len([word for word in window_words3 
                                             if word in allwords1['negative']])
                    
                    # Weigh by tfidf
                    conditional_deceppos = conditional_deceppos * tfidf3
                    conditional_decepneg = conditional_decepneg * tfidf3
                    
                    # Collect results
                    scores[title]['CSR_3'] += tfidf3
                    scores[title]['CSR_Deceptive_3'] += (conditional_deceppos - conditional_decepneg)
        ...
        for window in windows1_narr:
            ...
            if middle_bigram and middle_bigram in political_bigrams:
                tfidf1 = political_bigrams[middle_bigram]['tfidf']
                window_words1_narr = set([y for x in window for y in x.split()])
                
                conditional_deceppos_narr = len([word for word in window_words1_narr 
                                               if word in allwords1['positive']])
                conditional_decepneg_narr = len([word for word in window_words1_narr 
                                               if word in allwords1['negative']])
                
                conditional_deceppos_narr = conditional_deceppos_narr * tfidf1
                conditional_decepneg_narr = conditional_decepneg_narr * tfidf1
                
                scores[title]['CSR_1_narr'] += tfidf1
                scores[title]['CSR_Deceptive_1_narr'] += (conditional_deceppos_narr - conditional_decepneg_narr)
        ...
        for window in windows2_narr:
            ...
            if middle_bigram and middle_bigram in political_bigrams:
                ...
                if middle_bigram1 not in political_bigrams and middle_bigram2 not in political_bigrams:
                    tfidf2 = political_bigrams[middle_bigram]['tfidf']
                    window_words2_narr = set([y for x in window for y in x.split()])
                    
                    conditional_deceppos_narr = len([word for word in window_words2_narr 
                                                   if word in allwords1['positive']])
                    conditional_decepneg_narr = len([word for word in window_words2_narr 
                                                   if word in allwords1['negative']])
                    
                    conditional_deceppos_narr = conditional_deceppos_narr * tfidf2
                    conditional_decepneg_narr = conditional_decepneg_narr * tfidf2
                    
                    scores[title]['CSR_2_narr'] += tfidf2
                    scores[title]['CSR_Deceptive_2_narr'] += (conditional_deceppos_narr - conditional_decepneg_narr)
        ...
        for window in windows3_narr:
            ...
            if middle_bigram and middle_bigram in political_bigrams:
                ...
                if middle_bigram3 not in political_bigrams and middle_bigram4 not in political_bigrams:
                    tfidf3 = political_bigrams[middle_bigram]['tfidf']
                    window_words3_narr = set([y for x in window for y in x.split()])
                    
                    conditional_deceppos_narr = len([word for word in window_words3_narr 
                                                   if word in allwords1['positive']])
                    conditional_decepneg_narr = len([word for word in window_words3_narr 
                                                   if word in allwords1['negative']])
                    
                    conditional_deceppos_narr = conditional_deceppos_narr * tfidf3
                    conditional_decepneg_narr = conditional_decepneg_narr * tfidf3
                    
                    scores[title]['CSR_3_narr'] += tfidf3
                    scores[title]['CSR_Deceptive_3_narr'] += (conditional_deceppos_narr - conditional_decepneg_narr)
        ...
        for window in windows1_qa:
            ...
            if middle_bigram and middle_bigram in political_bigrams:
                tfidf1 = political_bigrams[middle_bigram]['tfidf']
                window_words1_qa = set([y for x in window for y in x.split()])
                
                conditional_deceppos_qa = len([word for word in window_words1_qa 
                                             if word in allwords1['positive']])
                conditional_decepneg_qa = len([word for word in window_words1_qa 
                                             if word in allwords1['negative']])
                
                conditional_deceppos_qa = conditional_deceppos_qa * tfidf1
                conditional_decepneg_qa = conditional_decepneg_qa * tfidf1
                
                scores[title]['CSR_1_qa'] += tfidf1
                scores[title]['CSR_Deceptive_1_qa'] += (conditional_deceppos_qa - conditional_decepneg_qa)
        ...
        for window in windows2_qa:
            ...
            if middle_bigram and middle_bigram in political_bigrams:
                ...
                if middle_bigram1 not in political_bigrams and middle_bigram2 not in political_bigrams:
                    tfidf2 = political_bigrams[middle_bigram]['tfidf']
                    window_words2_qa = set([y for x in window for y in x.split()])
                    
                    conditional_deceppos_qa = len([word for word in window_words2_qa 
                                                 if word in allwords1['positive']])
                    conditional_decepneg_qa = len([word for word in window_words2_qa 
                                                 if word in allwords1['negative']])
                    
                    conditional_deceppos_qa = conditional_deceppos_qa * tfidf2
                    conditional_decepneg_qa = conditional_decepneg_qa * tfidf2
                    
                    scores[title]['CSR_2_qa'] += tfidf2
                    scores[title]['CSR_Deceptive_2_qa'] += (conditional_deceppos_qa - conditional_decepneg_qa)
        ...
        for window in windows3_qa:
            ...
            if middle_bigram and middle_bigram in political_bigrams:
                ...
                if middle_bigram3 not in political_bigrams and middle_bigram4 not in political_bigrams:
                    tfidf3 = political_bigrams[middle_bigram]['tfidf']
                    window_words3_qa = set([y for x in window for y in x.split()])
                    
                    conditional_deceppos_qa = len([word for word in window_words3_qa 
                                                 if word in allwords1['positive']])
                    conditional_decepneg_qa = len([word for word in window_words3_qa 
                                                 if word in allwords1['negative']])
                    
                    conditional_deceppos_qa = conditional_deceppos_qa * tfidf3
                    conditional_decepneg_qa = conditional_decepneg_qa * tfidf3
                    
                    scores[title]['CSR_3_qa'] += tfidf3
                    scores[title]['CSR_Deceptive_3_qa'] += (conditional_deceppos_qa - conditional_decepneg_qa)
...
deceptivewords_file = 'C:/Users/Pratt/Desktop/申请内容/科研内容/Harry/deceptive_list_clean.csv'
polbigrams_file = 'C:/Users/Pratt/Desktop/申请内容/科研内容/Harry/cc_words.csv'
...
split_and_store_transcripts(transcripts_raw, 'seperate_files_small_initial')
seperate_files = glob.glob('seperate_files_small_initial/*_seperate.json')
processed_folder = 'processed_files_small_initial'
os.makedirs(processed_folder, exist_ok=True)

scores = {}
for seperate_file in seperate_files:
    file_scores = process_scores_file(seperate_file, processed_folder, political_bigrams, allwords1)
...
# Scale
scores_df["CSR"]=(scores_df["CSR_1"]+scores_df["CSR_2"]+scores_df["CSR_3"])/scores_df['Total words']
scores_df["CSR_narr"]=(scores_df["CSR_1_narr"]+scores_df["CSR_2_narr"]+scores_df["CSR_3_narr"])/scores_df['Total narr_words']
scores_df["CSR_qa"]=(scores_df["CSR_1_qa"]+scores_df["CSR_2_qa"]+scores_df["CSR_3_qa"])/scores_df['Total qa_words']
scores_df["CSR_Deceptive"]=(scores_df["CSR_Deceptive_1"]+scores_df["CSR_Deceptive_2"]+scores_df["CSR_Deceptive_3"])/scores_df['Total words']
scores_df["CSR_Deceptive_narr"]=(scores_df["CSR_Deceptive_1_narr"]+scores_df["CSR_Deceptive_2_narr"]+scores_df["CSR_Deceptive_3_narr"])/scores_df['Total narr_words']
scores_df["CSR_Deceptive_qa"]=(scores_df["CSR_Deceptive_1_qa"]+scores_df["CSR_Deceptive_2_qa"]+scores_df["CSR_Deceptive_3_qa"])/scores_df['Total qa_words']
scores_df["Deceptive"]=scores_df["Deceptive"]/scores_df['Total words'] 
scores_df["Deceptive_narr"]=scores_df["Deceptive_narr"]/scores_df['Total narr_words'] 
scores_df["Deceptive_qa"]=scores_df["Deceptive_qa"]/scores_df['Total qa_words'] 
